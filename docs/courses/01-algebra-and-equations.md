...
The restrictions are `x != 1` and `x != -1`. Multiply by `(x-1)(x+1)`:

`(x+1)+(x-1)=x^2-1`

So `2x=x^2-1`, giving `x^2-2x-1=0`. Hence `x=1+-sqrt(2)`. Neither value violates the restrictions.
...
Factor: `(x-2)(x-3) <= 0`. The parabola is negative between its roots, so the answer is `2 <= x <= 3`.

## Lesson 4: Powers, roots, and logarithms

Use the laws `a^m a^n=a^(m+n)`, `(a^m)^n=a^(mn)`, and `a^m/a^n=a^(m-n)`. For logarithms, `log_a(x)=b` means `a^b=x`, with `a>0`, `a != 1`, and `x>0`.

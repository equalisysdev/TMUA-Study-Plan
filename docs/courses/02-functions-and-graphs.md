...
For `f(x)=sqrt(7-x)`, the domain requires `7-x >= 0`, so `x <= 7`.
...
**Worked example:** Intersections of `y=1/x` and `y=x-2` satisfy `1/x=x-2`, with `x != 0`. Multiplying gives `x^2-2x-1=0`, so `x=1+-sqrt(2)`.
